    ...
    <div>
        ...
        <div class="container" style="max-width: 100%;">
            ...
            @foreach ($pedidos as $pedido)
                @php
                    $cor_fundo = '';
                    if ($pedido->statusPedido->id == 3) {
                        $cor_fundo = 'background-color: rgba(0, 128, 0, 0.49);';
                    } elseif ($pedido->statusPedido->id == 4) {
                        $cor_fundo = 'background-color: rgba(255, 0, 0, 0.50);';
                    }
                @endphp

                <div class="row" style="{{$cor_fundo}}">
                    ...
                    <div class="col col-lg-2" >
                        {{$pedido->statusPedido->status_pedido_atual}}
                    </div>
                    ...
                    @if(!in_array($pedido->statusPedido->id, [3, 4]))

                    <div class=" col-lg-1 ">
                        <a href="{{route('pedido.edit', $pedido -> id)}}"><button class="btn btn-warning fw-bold btn-sm">Editar</button></a>
                    </div>
                    @endif
